 ...
 *    class A2 를 만드는 팀과 class B2 를 만드는 팀을 분리해서
 *    각자가 개발을 진행할 수 있기 때문임.
 *    
 *    즉, 한 쪽에서 다른 쪽의 개발이 끝날 때 까지 기다리지 않아도 됨.
 * 
 * 2. 표준화가 가능 => 생산성과 관련됨.
 *    기본 틀을 인터페이스로 작성한 다음, 개발자들에게 인터페이스를 구현하도록 한다면,
 *    일관되고 정형화된 프로그램 개발이 가능해짐.
 * 
...
// class A 를 구현하는 팀원이 다른 팀원이 작성중인 class B2 의 완성까지 기다리지 않아도 됨.
